...
RSpec.describe 'Fonction de modèle de tâche', type: :model do
  describe 'Test de Validation' do
    context 'Si le Title de la tâche est une chaîne vide' do
      it 'Validation échoue' do
        task = Task.create(title: '', content: 'contenu', deadline_on: '2025/5/25', priority: 'medium', status: 'doing')
    ...
    context 'Si la description de la tâche est vide' do
      it 'Validation échoue' do
        task = Task.create(title: 'titre', content: '', deadline_on: '2025/5/25', priority: 'medium', status: 'doing')
        expect(task).not_to be_valid
      end
    end
    ...
    context 'Si le Title et la description de la tâche ont des valeurs' do
      it 'Vous pouvez enregistrer une tâche' do
        task = Task.create(title: 'titre', content: 'contenu', deadline_on: '2025/5/25', priority: 'medium', status: 'doing')
  ...
  describe 'search function' do
    ...
    let!(:third_task) { FactoryBot.create(:task, title: 'second task', deadline_on: '2025-02-16', priority: 'low', status: 'done') }
